...
class LayerPerformanceTracker:
    ...
    def get_detailed_breakdown(self) -> Dict:
        ...
        return {
            ...
            "phase_breakdown": {
                "total_wait_time_ms": self.phase_times['total_wait_time'] * 1000,
                "total_retrieve_time_ms": self.phase_times['total_retrieve_time'] * 1000,
                "total_verify_time_ms": self.phase_times['total_verify_time'] * 1000,
                "total_gpu_transfer_time_ms": self.phase_times['total_gpu_transfer_time'] * 1000,
                "total_accounted_time_ms": total_accounted_time * 1000,
            },
            
            # Per-layer breakdown
            "per_layer_wait_times": {k: v * 1000 for k, v in self.layer_wait_times.items()},
            "per_layer_retrieve_times": {k: v * 1000 for k, v in self.layer_retrieve_times.items()},
            "per_layer_verify_times": {k: v * 1000 for k, v in self.layer_verify_times.items()},
...
def run_receiver(args, config, metadata, tokens, retrieved_cache, slot_mapping, engine):
    ...
    for target_layer in range(metadata.kv_shape[0]):
        ...
        if engine.layer_status.wait_for_layer_busy(target_layer, timeout_us=10000000):  # 10s timeout per layer
            ...
            logger.debug(f"   ⚡ Layer {target_layer}: Ready after {(time.time() - layer_start)*1000:.2f}ms wait")
            
            # Track retrieval time
            retrieve_start = perf_tracker.record_layer_retrieve_start(target_layer)
            ...
            perf_tracker.record_layer_retrieve_end(target_layer, retrieve_start)
            
            if ret_mask is not None:
                retrieved_tokens = torch.sum(ret_mask).item()
                
                # Track verification time
                verify_start = perf_tracker.record_layer_verify_start(target_layer)
                
                # Verify the layer data
                if verify_layer_pattern(retrieved_cache, slot_mapping, target_layer, tolerance=0.01):
                    verification_results[target_layer] = "✅ PASS"
                else:
                    verification_results[target_layer] = "❌ FAIL"
                
                perf_tracker.record_layer_verify_end(target_layer, verify_start)
                ...
                wait_time = perf_tracker.layer_wait_times.get(target_layer, 0) * 1000
                retrieve_time = perf_tracker.layer_retrieve_times.get(target_layer, 0) * 1000
                verify_time = perf_tracker.layer_verify_times.get(target_layer, 0) * 1000
                
                logger.info(f"   ✨ Layer {target_layer}: {retrieved_tokens} tokens | "
            ...
            logger.warning(f"   ⏰ Layer {target_layer}: Timeout after {wait_time:.1f}ms wait")
            break
    ...
    if detailed_metrics["status"] != "no_data":
        ...
        logger.info(f"   Total wait time: {phase_breakdown['total_wait_time_ms']:.2f}ms")
        logger.info(f"   Total retrieve time: {phase_breakdown['total_retrieve_time_ms']:.2f}ms")
        logger.info(f"   Total verify time: {phase_breakdown['total_verify_time_ms']:.2f}ms")
        logger.info(f"   Total accounted time: {phase_breakdown['total_accounted_time_ms']:.2f}ms")
        logger.info(f"   Unaccounted time: {(total_time*1000) - phase_breakdown['total_accounted_time_ms']:.2f}ms")
        
        # Phase percentages
        total_time_ms = total_time * 1000
        if total_time_ms > 0:
            wait_pct = (phase_breakdown['total_wait_time_ms'] / total_time_ms) * 100
            retrieve_pct = (phase_breakdown['total_retrieve_time_ms'] / total_time_ms) * 100
            verify_pct = (phase_breakdown['total_verify_time_ms'] / total_time_ms) * 100
            ...
            logger.info(f"   🚨 BOTTLENECK: {max_phase[0]} phase ({max_phase[1]:.2f}ms)")
        ...
        if wait_times:
            slowest_wait_layers = sorted(wait_times.items(), key=lambda x: x[1], reverse=True)[:5]
        ...
        if retrieve_times:
            slowest_retrieve_layers = sorted(retrieve_times.items(), key=lambda x: x[1], reverse=True)[:5]
            logger.info(f"\n📥 SLOWEST RETRIEVE TIMES:")
            for layer_id, retrieve_time in slowest_retrieve_layers:
                logger.info(f"   Layer {layer_id}: {retrieve_time:.2f}ms retrieve")
    ...
    if total_time > 0 and len(tokens) > 0:
        ...
        total_chunks_received = len(processed_layers) * (len(tokens) // config.chunk_size)
        total_bytes_received = total_chunks_received * chunk_size_bytes
        
        receiver_throughput = calculate_throughput(total_bytes_received, total_time)
        ...
        logger.info(f"   Total data received: {total_bytes_received / (1024**3):.3f} GB")
    ...
    if len(processed_layers) > 1:
        ...
        parallel_benefit = ((sequential_time - actual_time_ms) / sequential_time) * 100
        
        # Detailed calculation breakdown
        logger.info(f"   === Latency Benefit Calculation ===")
        logger.info(f"   Avg time per layer: {metrics['avg_time_per_layer_ms']:.2f}ms")
        logger.info(f"   Number of processed layers: {len(processed_layers)}")
        logger.info(f"   Estimated sequential time: {metrics['avg_time_per_layer_ms']:.2f} × {len(processed_layers)} = {sequential_time:.2f}ms")
        logger.info(f"   Actual parallel time: {actual_time_ms:.2f}ms")
        logger.info(f"   Time difference: {sequential_time:.2f} - {actual_time_ms:.2f} = {sequential_time - actual_time_ms:.2f}ms")
        logger.info(f"   Benefit calculation: ({sequential_time - actual_time_ms:.2f} / {sequential_time:.2f}) × 100 = {parallel_benefit:.1f}%")
        if parallel_benefit < 0:
            slowdown_factor = actual_time_ms / sequential_time
            logger.info(f"   🚨 PERFORMANCE WARNING: Parallel processing is {slowdown_factor:.1f}x SLOWER than sequential!")
        ...
        if detailed_metrics["status"] != "no_data":
            ...
            if wait_time_ms > total_time_ms * 0.8:
                logger.info(f"   Issue: Excessive waiting time ({wait_time_ms:.1f}ms / {total_time_ms:.1f}ms = {wait_time_ms/total_time_ms*100:.1f}%)")
                logger.info(f"   Likely cause: Network latency, sender not transmitting layers fast enough, or timeout issues")
                logger.info(f"   Fix: Check sender throughput, reduce timeouts, or optimize RDMA transfer")
            elif phase_breakdown['total_retrieve_time_ms'] > total_time_ms * 0.5:
                logger.info(f"   Issue: Slow data retrieval")
                logger.info(f"   Likely cause: GPU memory bottleneck or storage backend latency")
                logger.info(f"   Fix: Optimize GPU connector or storage backend")
            elif len(processed_layers) < metadata.kv_shape[0] * 0.5:
                logger.info(f"   Issue: Many layers failed to process ({len(processed_layers)}/{metadata.kv_shape[0]})")
                logger.info(f"   Likely cause: Timeouts or data corruption")
                logger.info(f"   Fix: Increase timeout values or check data integrity")
            else:
                logger.info(f"   Issue: General overhead - layers processed sequentially instead of in parallel")
                logger.info(f"   Likely cause: Lock contention or blocking operations")
                logger.info(f"   Fix: Profile thread contention and optimize synchronization")

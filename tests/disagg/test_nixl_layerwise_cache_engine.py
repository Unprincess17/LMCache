...
from lmcache.experimental.gpu_connector import VLLMPagedMemLayerwiseGPUConnector
...
def create_config(role: str, host: str, port: int) -> LMCacheEngineConfig:
    """Create a configuration for the LayerwiseLMCacheEngine with Nixl backend."""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Test LayerwiseLMCacheEngine with Nixl backend')
    ...
    for round_num in range(args.num_rounds):
        ...
        if args.role == "sender":
            ...
            # Use store_layer generator for layerwise storage
            ...
            if received_count == args.num_chunks:
                ...
                # Use retrieve_layer generator for layerwise retrieval
                ret_mask = None
                retrieve_generator = engine.retrieve_layer(
                    tokens,
                    kvcaches=retrieved_cache,
                    slot_mapping=slot_mapping)
                for result in retrieve_generator:

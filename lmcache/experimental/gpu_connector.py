...
from lmcache.utils import _lmcache_nvtx_annotate, NVTXContext
...
class VLLMPagedMemLayerwiseGPUConnector(GPUConnectorInterface):
    ...
    def from_gpu_to_combined_memory(self, combined_memory_obj: MemoryObj,
        ...
        assert layer_id < len(
            kvcaches), f"layer_id {layer_id} exceeds available layers"

        # Calculate total tokens and prepare slot mapping
        total_tokens = sum(
            offset_end - offset_start
            for _, _, _, offset_start, offset_end in chunk_offsets)
        ...
        if tmp_gpu_buffer_obj is None:
            raise RuntimeError(
                "Failed to allocate GPU buffer for combined memory transfer")
        ...
        logger.debug(
            f"Successfully transferred {len(chunk_offsets)} chunks to combined memory for layer {layer_id}"
        )

    @_lmcache_nvtx_annotate
    def to_gpu_single_chunk(self, chunk_tensor: torch.Tensor, start: int,
                            end: int, layer_id: int, **kwargs):
        """
        ...
        if tmp_gpu_buffer_obj is None:
            raise RuntimeError(
                "Failed to allocate GPU buffer for single chunk transfer")
        ...
        try:
            ...
            with torch.cuda.stream(self.load_stream):
                ...
                tmp_gpu_buffer_obj.tensor.copy_(chunk_tensor,
                                                non_blocking=True)
        ...
        logger.debug(
            f"Transferred single chunk ({num_tokens} tokens) to GPU layer {layer_id}"
        )
    ...
    def to_gpu_from_combined_memory(self, combined_memory_obj: MemoryObj,
        ...
        if tmp_gpu_buffer_obj is None:
            raise RuntimeError(
                "Failed to allocate GPU buffer for combined memory retrieval")
        ...
        logger.debug(
            f"Successfully transferred {len(chunk_offsets)} chunks from combined memory to GPU for layer {layer_id}"
        )
